...
## style 中的 scope 属性
在 Vue 中，style 标签的 scope 属性用于将样式限定在当前组件中，避免样式污染其他组件。当使用 scoped 属性时，Vue 会自动为当前组件的元素添加一个唯一的属性，例如 data-v-123456，然后将样式规则中的选择器转换为类选择器，并添加 data-v-123456 属性，从而确保样式只作用于当前组件的元素。
