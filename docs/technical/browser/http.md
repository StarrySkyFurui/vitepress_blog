...
### HTTP 1.0
- 支持基本的请求方法（GET、POST、HEAD）
- 无状态，每个请求都是独立的
- 支持基本的身份验证和状态管理（通过Cookie）
- 消息结构包含请求头和请求体
...
### HTTP 2.0
- 用二进制格式而非文本格式，将HTTP消息分解为更小的帧，提高了传输效率
- 多路复用，允许在一个TCP连接上同时发送多个请求和响应，避免了队头阻塞问题
- 头部压缩，通过HPACK算法压缩HTTP头部，减少了传输的开销
- 服务器推送，服务器可以在客户端请求之前主动推送资源，提高了页面的加载速度
- 支持流优先级，允许客户端指定请求的优先级，服务器可以根据优先级进行资源分配

### HTTP 3.0
- 基于QUIC协议，QUIC协议是一个基于UDP的传输层协议，提供了可靠性和安全性，避免了TCP的延迟和丢包问题
- 头部压缩，通过QPACK算法压缩HTTP头部，减少了传输的开销
- 支持快速握手，通过0-RTT或1-RTT握手，实现了快速建立连接
- 支持流量控制和拥塞控制，提高了网络传输的稳定性和效率
