## 浏览器的主要组件
- 用户界面
    包括地址栏、后退/前进按钮、书签目录等。这些是除了浏览器主窗口显示请求页面外，其它现实的各个部分都属于用户界面。
- 浏览器引擎
    在⽤户界⾯和呈现引擎之间传送指令。
- 渲染引擎
    负责显示请求的内容。例如，如果请求内容为 `HTML`，它负责解析 `HTML` 及 `CSS`，并显示解析后的结果。
- 网络
    用于完成网络调用，如 `HTTP` 请求。它具有平台无关的接口，并为所有平台提供底层实现，可在不同平台上工作。
- ⽤户界⾯后端
    ⽤于绘制基本的窗⼝⼩部件，⽐如组合框和窗⼝。其公开了与平台⽆关的通⽤接⼝，⽽在底层使⽤操作系统的⽤户界⾯⽅法。
- JS解释器
...
## 浏览器的缓存机制
浏览器缓存机制主要用于提高网页加载速度，减少网络流量，减轻服务器负担。根据缓存的处理方式不同，主要分为两种类型：强缓存和协商缓存。

### 强缓存
强缓存：状态码为 200， 是指浏览器在加载资源时，会先根据 `HTTP` 头信息判断资源是否命中强缓存，如果命中则直接使用缓存中的资源，否则才会去服务器请求数据
* 控制字段
    - Expires：指定缓存过期时间，是一个绝对时间，如 `Expires: Fri, 30 Oct 2019 23:59:59 GMT`
    - Cache-Control：指定缓存控制，包含多个指令，如 `Cache-Control: max-age=3600` 表示资源的有效时间。
如果 cache-control 与 expires 同时存在的话， cache-control 的优先级高于 expires。

### 协商缓存
协商缓存：状态码为 304， 是指浏览器在发送请求时会带上缓存标识（如 Last-Modified 或 ETag），服务器会根据这些标识判断资源是否已经发生变化。
* 控制字段
    - Last-Modified / If-Modified-Since：指定资源最后修改时间，如 `Last-Modified: Fri, 30 Oct 2019 23:59:59 GMT`
    - ETag / If-None-Match：指定资源的唯一标识，如 `ETag: "5b6a8c21-1a1f-4274-b6ae-6b3d6b33c8d4"`


### 工作流程
工作流程大致如下图所示：
![cache.png](../../images/technical/browser/cache.png)
#### 1. 首次请求：
* 浏览器发送请求到服务器。
* 服务器响应带有缓存控制字段的资源。

#### 2. 强缓存命中：
* 如果资源未过期（根据 Expires 或 Cache-Control 判断），则浏览器直接从本地缓存加载资源，不向服务器发送请求。

#### 3. 协商缓存过程：
* 如果资源已过期或没有强缓存控制字段，浏览器会带上缓存标识（If-Modified-Since 或 If-None-Match）向服务器发送请求。
* 服务器收到请求后，检查资源是否已更改。
* 如果资源未更改，服务器返回状态码 304 Not Modified，浏览器使用本地缓存。
* 如果资源已更改，服务器返回新的资源和状态码 200 OK。

### 优缺点
* 强缓存：
    - 优点：完全避免了服务器往返时间，加载速度最快。
    - 缺点：可能导致缓存资源过期后仍然被使用。

* 协商缓存：
    - 优点：可以保证缓存资源是最新的，减少了过期资源的问题。
    - 缺点：每次请求都需要与服务器交互，增加了网络开销。
...
## 垃圾回收机制
###  垃圾回收的概念
Js 引擎在运行时，需要分配内存空间来存储变量和值。当变量不再参与运行的时候，就需要系统收回被占用的内存空间。

### 垃圾回收的方式
1. 标记清除
   当变量进入执行环境时，就标记这个变量“进入环境”，被标记为“进入环境”的变量是不能被回收的，因为它们正在被使用。当变量离开执行环境时，就标记这个变量“离开环境”，被标记为“离开环境”的变量会被回收。
   垃圾收集器在运行的时候会给存储在内存中的所有变量都加上标记。容纳安徽，它会去掉环境中的变量以及被环境中的变量引用的标记。而在此之后再被加上标记的变量将被视为准备删除的变量，原因是环境中的变量也已经无法访问到这些变量了。最后垃圾收集器完成内存清除工作，销毁那些带标记的值，病挥挥手它们所占用的内存空间。
2. 引用计数（Reference Counting）
    引用计数就是跟踪记录每个值被引用的次数。当声明了一个变量并将一个引用类型赋值给该变量时，则这个值的引用次数就是1.相反，如果包含对这个值引用的变量又取得了另一个值，则这个值得引用次数就减1。当这个值得引用次数变成0时，则说明这个值没有价值了。因此在垃圾收集器上，当发现有一个对象的引用次数为0时，就可以将其所占用的内存空间回收。
    * 这种方法会引起**循环引用**的问题，即两个对象相互引用，导致它们的引用次数永远不会为0，从而无法被垃圾回收机制回收。这种情况下，就需要手动释放变量占用的内存。
