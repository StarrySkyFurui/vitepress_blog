## Vue 的响应式原理
### 1.数据劫持：
* Vue 使用 `Object.defineProperty()` 方法（在 Vue 2.x 中）或 `Proxy`（在 Vue 3.x 中）来劫持对象属性的 `getter` 和 `setter`。
* 当组件渲染时，`Vue` 会遍历 `data` 对象中的每一个属性，并使用 `Object.defineProperty()` 或 `Proxy` 为其添加 `getter` 和 `setter`。

### 2.依赖收集：
* 当组件渲染时，如果遇到使用了 `data` 中属性的地方（例如通过插值表达式 `{{ message }}`），`Vue` 就会创建一个 `Watcher` 实例（观察者）。
* 这个 `Watcher` 实例会把自己添加到对应属性的依赖数组中，表示这个属性将来如果发生变化，需要通知这个 `Watcher` 去更新视图。

### 3.发布-订阅模式：
* 当 `data` 中的属性发生变化时（即 setter 被触发），`Vue` 会遍历该属性的依赖数组，通知所有的 `Watcher`。
* `Watcher` 收到通知后，会重新计算数据并更新组件的 `DOM`。

### 4.组件更新：
* 由于 `Vue` 中的组件是响应式的，所以当组件的 `props`、`data`、`computed` 属性或 `methods` 中的方法被依赖的数据更新时，组件会重新渲染。
* 这种机制避免了手动操作 `DOM`，提高了开发效率。

### Vue 3.x 的改进：
* Vue 3.x 使用了 `Proxy` 来替代 Vue 2.x 中的 `Object.defineProperty()`。
* `Proxy` 提供了更完整的属性拦截（包括数组和新增属性），性能也更好。

### 总结：
* `Vue` 的响应式原理是通过数据劫持、依赖收集和发布-订阅模式来实现的。
* 当数据发生变化时，`Vue` 能够自动更新相关的组件视图，从而实现了响应式编程。

## Vue 的组件生命周期
   
* Vue2 包含`created`（创建阶段）、`mounted`（挂载阶段）、`updated`（更新阶段）、`beforeDestroy`（销毁阶段）等生命周期钩子。
* Vue3 对生命周期进行了调整，移除了部分生命周期钩子，如 `beforeCreate` 和 `destroyed`，并引入了新的 `onBeforeMount`、`onMounted`、`onBeforeUpdate`、`onUpdated`、`onBeforeUnmount`、`onUnmounted` 等。
...
## Vue 中的 diff 算法
用于比较新旧虚拟 DOM（VNode）树，并找出它们之间的差异，从而高效地更新真实 DOM 的算法。
### 算法特点
* 同层比较：diff 算法的比较只会在同层级进行，不会跨层级比较。
* 双指针策略：比较过程中，diff 算法采用双指针策略，从新旧节点的首尾向中间进行比较，以最小化比较次数。
### 比较方式
* 节点比较：新旧节点进行比较，如果节点类型不同，则直接替换旧的节点。
* key 属性比较：如果节点类型相同，但设置了 key 属性，则根据 key 判断是否是同一个节点。如果 key 值不同，也会认为这两个节点是不同的，并进行替换。
* 属性比较：对于类型相同且 key 值也相同的节点，Vue 会进一步比较它们的属性是否发生变化。如果属性发生变化，会更新节点的属性。
* 子节点比较：如果节点有子节点，Vue 会递归地对子节点进行 diff 算法比较。

### 算法流程
* 遍历新旧节点，对相同位置的节点进行比较。
* 如果新旧节点类型不同，则直接替换旧节点。
* 如果新旧节点类型相同，且设置了 key 属性，则根据 key 判断是否是同一个节点。
* 如果新旧节点类型相同，且 key 值也相同，则进一步比较它们的属性是否发生变化。
* 如果新旧节点都有子节点，则递归地对子节点进行 diff 算法比较。
* 如果新旧节点都没有子节点，则直接替换旧节点。
* 更新真实 DOM。

## Vue 中的事件修饰符
在 Vue.js 中，事件修饰符（Event Modifiers）是一种特殊的语法糖，用于在监听 DOM 事件时添加一些额外的功能，而不需要显式地调用如 event.preventDefault() 或 event.stopPropagation() 这样的方法。
### .stop
* 作用：阻止事件冒泡。
* 示例：
```vue
...
```
* 注意：`.stop` 修饰符会阻止事件冒泡，但不会阻止事件捕获。

### .prevent
* 作用：阻止默认事件行为。等同于 event.preventDefault()。
* 示例：
```vue
...
```
* 注意：`.prevent` 修饰符会阻止默认事件行为，但不会阻止事件冒泡。

### .capture
* 作用：使用事件捕获模式。事件将会先被父元素捕获，然后再触发子元素。
* 示例：
```vue
...
### .self
* 作用：只有当事件在元素自身（而不是子元素）触发时，才会触发事件处理器。
* 示例：
```vue
...
### .{keyCode 或 keyAlias}
* 作用：只当事件是从特定键触发时才触发回调。
* 示例：
```html
<!-- 当按下回车键时才调用 `vm.submit()` -->
...
<img :src="imageSrc" />
```
* 注意：当在 `DOM` 中插入值时 (例如：v-html) ，`v-bind` 优先于表达式具有更高的优先级。

### v-model:

* 作用：创建双向数据绑定，使得表单输入或文本域的值和 Vue 实例的数据属性保持同步。
* 示例：
```vue
...
```
* 注意：在 `v-for` 中，建议使用 `:key `绑定一个唯一的值，以提高渲染性能。
...
* 作用：根据表达式的值的真假条件性地渲染元素。
* 示例：
```vue
<div v-if="isVisible">显示这个</div>
<div v-else>显示那个</div>
```
* 注意：`v-if` 是“真正”的条件渲染，因为它会确保在切换过程中条件块内的事件监听器和子组件适当地被销毁和重建。
...
* 示例：
```vue
<div v-show="isVisible">显示这个</div>
```
* 注意：`v-show` 简单地基于 `CSS` 进行切换。元素始终都会被渲染并保持在 `DOM` 中。
...
* 示例：
```vue
<span v-text="message">span元素</span>
```
* 注意：它会覆盖元素内原有的内容。
...
```
* 注意：使用 `v-html` 插入 `HTML` 可能会导致 `XSS` 攻击，因此仅对可信内容使用它。
...
```
* 注意：这个指令可以用于解决插值表达式闪烁的问题。

### v-once:

* 作用：将元素和组件只渲染一次。随后的重新渲染，元素/组件及其所有的子节点将被视为静态内容并跳过。这可以用于优化更新性能。
* 示例：
```vue
<div v-once>{{ message }}</div>
```
* 注意：这个指令不能和 `v-for` 一起使用，因为 `v-for` 的优先级比 `v-once` 高。
...
## Vue 中的过滤器
过滤器（Filters）是一种用于在文本渲染到 DOM 之前对其进行格式化的功能。然而，需要注意的是，在 Vue 2.x 中，过滤器是存在的，但在 Vue 3.x 中，过滤器已经被移除，因为 Vue 团队认为它们不是响应式的，并且可以用计算属性或方法更好地处理这些任务。
### Vue 2.x 中的过滤器
在 Vue 2.x 中，你可以使用 Vue.filter() 方法来全局注册一个过滤器，或者在组件的 filters 选项中局部注册一个过滤器。

以下是一个 Vue 2.x 中过滤器的示例：
```js
// 全局注册一个名为 'uppercase' 的过滤器
...
```
在这个例子中，uppercase 过滤器会将任何传递给它的值转换为大写。在模板中，我们使用管道符 | 来应用这个过滤器到 message 变量上。

### Vue 3.x 中的过滤器
由于 Vue 3.x 已经移除了过滤器，你可以使用计算属性（Computed Properties）或方法（Methods）来实现类似的功能。
```vue
<template>
  <div id="app">
    {{ uppercasedMessage }}
...
```
在这个例子中，uppercasedMessage 计算属性将 message 的值转换为大写，并在模板中直接使用这个计算属性的值。
...
## 虚拟 DOM
虚拟 DOM 是 `Vue` 框架中一个非常重要的概念，它是一种在内存中模拟真实 DOM 的数据结构。通过使用虚拟 DOM，我们可以将 DOM 操作从浏览器的真实 DOM 转移到内存中的虚拟 DOM 上进行操作。这样就可以避免频繁地访问真实 DOM，从而提高性能和响应速度。
### 原理
1. 创建虚拟节点
应用启动时，框架会根据组件的结构和属性创建一个由 JavaScript 对象构成的树状结构，这个树即为虚拟 DOM。每个节点包含标签名、属性、子节点等信息。
2. 比较虚拟节点
当应用状态变化导致需要更新 UI 时，框架首先会重新计算生成新的虚拟 DOM 树。接着，利用高效的算法（如 Diff 算法）比较新旧虚拟 DOM 树的差异。
3. 更新真实 DOM
仅将发现的最小差异部分应用到实际的 DOM 上，而非全部重新渲染。这样避免了不必要的 DOM 操作，提升了性能。
### 优点
* 性能优化：减少实际 DOM 操作次数，以避免不必要的 DOM 操作，从而提高性能。
* 跨平台兼容：虚拟 DOM 抽象了底层实现，使得框架可以在不同平台上运行，而无需考虑底层平台的差异。
* 开发效率：虚拟 DOM 使得开发者可以专注于逻辑层的开发，而无需关心底层的 DOM 操作。
* 代码简洁：使用虚拟 DOM 可以让开发者更专注于业务逻辑和视图结构的编写，而无需过多关心底层的 DOM 操作。

## Vue3 组合式函数
组合式函数主要通过 setup() 函数来定义，该函数是 Vue 3 组件选项中的一个新属性，它在组件创建之前被调用，并且提供了响应式状态、计算属性、方法等。
### 定义
组合式函数通常指的是在 setup() 函数中使用的 Vue 提供的响应式 API，如 ref(), reactive(), computed(), watch(), watchEffect(), provide(), inject() 等。
### 注意事项
* setup() 函数在组件创建之前被调用，此时组件的 this 上下文还不存在，因此你不能在 setup() 中使用 this。
* setup() 函数的返回值会暴露给模板，你可以直接在模板中访问这些返回的属性或方法。
* ref() 用于创建基本数据类型的响应式引用，reactive() 用于创建对象的响应式副本。
* computed() 返回一个计算属性，该属性是基于其依赖项进行缓存的。
* watch() 和 watchEffect() 用于观察响应式数据的变化并执行相应的逻辑。
* provide() 和 inject() 用于在父子组件之间建立依赖关系，类似于传统的 props 和 events，但它们可以在更深层次的组件树中进行通信。
...
## Vue 中 scoped styles 样式隔离
在 Vue 中，.vue 单文件组件的 `<style>` 标签可以添加一个 `scoped` 属性来实现样式隔离，通过这个属性，Vue 会确保样式只应用到当前组件的模板中，而不会泄露到外部的其它组件中。

### 工作原理
* 唯一选择器：当 Vue 编译单文件组件时，如果 <style> 标签中包含 scoped 属性，Vue 会为每个样式选择器生成一个唯一的属性选择器。这个唯一的选择器通常是形如 [data-v-xxxxxxx] 的属性选择器，其中 xxxxxxx 是一个唯一的标识符（在 Vue Loader 用 PostCSS 来处理 scoped 的 CSS，在构建过程中生成的。它为组件模板内的每个元素添加一个独特的属性是一个基于组件文件内容的的 hash 值， 确保了它的唯一性，即使有多个相同的组件实例，每个实例的 hash 值也不同，因为它们在内存在的位置不同）。

* 编译时转换：在编译过程中，Vue 解析组件的模板并处理样式。对于具有 scoped 属性的样式，Vue 会将选择器转换为带有唯一属性选择器的形式，例如 .class 转换为 .class[data-v-xxxxxxx]。

* 渲染时应用：在组件渲染过程中，Vue 会为组件的根元素添加一个属性，其值为唯一标识符，例如 data-v-xxxxxxx。当组件渲染完成后，样式选择器中的唯一属性选择器会与组件根元素的属性匹配，从而实现样式的隔离。

### 样式穿透
如果需要在 scoped 样式中影响子组件的样式，可以通过使用 深度选择器（::v-deep）或 >>> 伪类选择器来实现，它们允许你穿透样式隔离。
