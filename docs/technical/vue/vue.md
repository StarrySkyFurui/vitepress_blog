...
### 工作原理
* 唯一选择器：当 Vue 编译单文件组件时，如果 <style> 标签中包含 scoped 属性，Vue 会为每个样式选择器生成一个唯一的属性选择器。这个唯一的选择器通常是形如 [data-v-xxxxxxx] 的属性选择器，其中 xxxxxxx 是一个唯一的标识符（在 Vue Loader 在构建过程中生成的，是一个基于组件文件内容的的 hash 值， 确保了它的唯一性，即使有多个相同的组件实例，每个实例的 hash 值也不同，因为它们在内存在的位置不同）。
